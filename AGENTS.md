# AGENTS.md — instructions for an AI agent working in this repo
...
| `dot_claude/skills/` | Codex skills, currently `c4-architect`. The **only** managed path under `~/.Codex` — `.chezmoiignore` denies the rest of that tree, which holds session transcripts and memory files |
